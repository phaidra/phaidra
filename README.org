...
* Apache image
  ...
  #+end_example
* Apache Notes
  + Apache needs some proxy modules to be enabled, one can activate it by running ~sudo a2enmod proxy proxy_http~.
  + Also, for development listening on port 8899 will need ~echo "Listen 8899" | sudo tee -a /etc/apache2/ports.conf~.
