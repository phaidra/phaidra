...
* System startup
** testing/dev
  To start up a local testinstance of phaidra, which will run on http://localhost:8899 you need this repo on your computer and then run ~docker compose up -d~ from the base-directory of the repo.  Depending on your internet connection and PC power, the set up  will last about 10-30min.
** productive/ssl
  ...
  Once you've got these prerequisites, change into the ~prod~ directory of this repo, put FQDN and IP into the lower section of the ~.env~ file (in the ~prod~ directory) and run ~docker compose up -d~ from there.
