...
* TODOs
  + resolve ui dependence on reverse proxy redirecting requests to /detail/$PID from API download.
  + track down ui 'Other links' URL storage location (changing baseurl in PhaidraAPI.json does not change behaviour, only clearing persistent data does -- 'other links' completely disappears then).
