<template>
  <v-container fluid>
        ...
        >
          <template #item="{ props, internalItem }">
            <v-list-item v-bind="props" lines="two">
              ...
              <v-list-item-subtitle v-html="`${internalItem.raw['@id']}`"></v-list-item-subtitle>
            </v-list-item>
          </template>
    ...
    <v-row>
      <v-col cols="2">
        <p-i-select
          v-bind="objectypefield"
          v-on:input="selectInput(objectypefield, $event)"
        ></p-i-select>
      </v-col>
      <v-col cols="5">
        <p-i-association
          v-bind="associationfield"
    ...
    <v-row>
      <v-col cols="12">
        <v-card>
          <v-card-title class="text-title-large font-weight-light text-white">{{ $t('Metadata preview') }}</v-card-title>
          <v-card-text>
