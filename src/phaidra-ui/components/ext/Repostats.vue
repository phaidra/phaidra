<template>
  <v-container>
    <v-row justify="center">
      <v-col>
        <v-card tile>
          <v-card-title class="title font-weight-light grey white--text">{{ $t('Repository statistics') }}</v-card-title>
          <v-card-text>
            <v-data-table
              :headers="typeHeaders"
              :items="typeItems"
  ...
  methods: {
    async fetchStats (self) {
      ...
      for (let i = fromYear; i <= toYear; i++) {
        ...
        this.cmodelStorageHeaders.push({ text: i.toString(), value: i.toString() })
        this.ownerHeaders.push({ text: i.toString(), value: i.toString() })
      }
      this.cmodelStorageHeaders.push({ text: 'total', value: 'total' })
      for (let term of this.vocabularies['objecttypeuwm'].terms) {
        let params = {
          q: '*:*',
        ...
        try {
          ...
          if (response.data.facet_counts.facet_ranges.tcreated.counts) {
            ...
            for (let j = 0; j < a.length; j = j + 2) {
              ...
              stats.objectType = this.getLocalizedTermLabel('objecttypeuwm', term['@id'])
      ...
      for (let term of this.vocabularies['objecttype'].terms) {
        let params = {
          q: '*:*',
          fq: 'object_type_id:"' + term['@id'] + '"',
          facet: 'on',
      ...
      for (let term of this.vocabularies['cmodels'].terms) {
        let params = {
          q: '*:*',
          fq: 'cmodel:"' + term['@id'] + '"',
          facet: 'on',
      ...
      try {
        ...
        if (response.data.stats) {
          ...
          for (let s of response.data.stats) {
            if ((s.model !== 'Container') && (s.model !== 'Collection')) {
              if (s.size > 0) {
                ...
                stats[s.model][s.upload_date] = (s.size / Math.pow(1024, Math.floor(3))).toFixed(3)
          ...
          for (let i = fromYear; i <= toYear; i++) {
            stats.total[i] = (stats.total[i] / Math.pow(1024, Math.floor(3))).toFixed(3)
          }
          for (let cmodel of Object.keys(stats)) {
            stats[cmodel].total = (stats[cmodel].total / Math.pow(1024, Math.floor(3))).toFixed(3)
