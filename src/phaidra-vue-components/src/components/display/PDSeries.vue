      ...
      <v-card tile elevation="0" color="transparent">
        <v-card-text class="jsonld-border-left">
            <v-row>
              <template v-for="(title, j) in o['dce:title']" :key="'title' + j">
                <template v-for="(mt, i) in title['bf:mainTitle']" :key="'mt' + j + '-' + i">
                  <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ $t(title['@type']) }}<template v-if="showLang && mt['@language']"> ({{ mt['@language'] }})</template></v-col>
                  <v-col :md="valueColMd" cols="12" class="valuefield">{{ mt['@value'] }}</v-col>
                </template>
              </template>
            </v-row>
            <v-row v-for="(volume, i) in o['bibo:volume']" :key="'volume'+i">
              <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ $t('Volume') }}<template v-if="showLang && volume['@language']"> ({{ volume['@language'] }})</template></v-col>
              <v-col :md="valueColMd" cols="12" class="valuefield">{{ volume['@value'] || volume }}</v-col>
            </v-row>
            <v-row v-for="(issue, i) in o['bibo:issue']" :key="'issue'+i">
              <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ $t('Issue') }}<template v-if="showLang && issue['@language']"> ({{ issue['@language'] }})</template></v-col>
              <v-col :md="valueColMd" cols="12" class="valuefield">{{ issue['@value'] || issue }}</v-col>
            </v-row>
            <v-row v-for="(issn, i) in o['ids:issn']" :key="'issn'+i">
              <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ $t('ISSN') }}</v-col>
              <v-col :md="valueColMd" cols="12" class="valuefield">{{ issn }}</v-col>
            </v-row>
            <v-row v-for="(issued, i) in o['dcterms:issued']" :key="'issued'+i">
              <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold" >{{ $t('Issued') }}</v-col>
            ...
            <v-row v-for="(id, i) in o['skos:exactMatch']" :key="'seriesexactMatch'+i">
              <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ getLocalizedTermLabel('objectidentifiertype', id['@type']) }}</v-col>
