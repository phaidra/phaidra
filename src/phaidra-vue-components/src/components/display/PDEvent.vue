          ...
          <v-container fluid>
            <v-row>
              <template v-for="(name, i) in o['skos:prefLabel']" :key="'name' + i">
                <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ $t('Name') }}<template v-if="showLang && name['@language']"> ({{ name['@language'] }})</template></v-col>
                <v-col :md="valueColMd" cols="12" class="valuefield">{{ name['@value'] }}</v-col>
              </template>
            </v-row>
            <v-row>
              <template v-for="(d, i) in o['rdfs:comment']" :key="'comment' + i">
                <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ $t('Event description') }} ({{ d['@language'] }})</v-col>
            ...
            <v-row>
              <template v-for="(location, j) in o['ebucore:hasEventRelatedLocation']" :key="'loc' + j">
                <template v-for="(place, i) in location['skos:prefLabel']" :key="'place' + j + '-' + i">
                  <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ $t('Location') }}</v-col>
            ...
            <v-row>
              <template v-for="(d, i) in o['ebucore:eventStartDateTime']" :key="'start' + i">
                <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ $t('Start date') }}</v-col>
                <v-col class="valuefield" :md="valueColMd" cols="12">{{ d }}</v-col>
              </template>
            </v-row>
            <v-row>
              <template v-for="(d, i) in o['ebucore:eventEndDateTime']" :key="'end' + i">
                <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ $t('End date') }}</v-col>
                <v-col class="valuefield" :md="valueColMd" cols="12">{{ d }}</v-col>
              </template>
            </v-row>
            <v-row>
              <template v-for="(id, i) in o['skos:exactMatch']" :key="'id' + i">
                <v-col :md="labelColMd" cols="12" class="pdlabel text-secondary font-weight-bold">{{ getLocalizedTermLabel('entityidentifiertype', id['@type']) }}</v-col>
