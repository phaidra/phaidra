            ...
            <v-row>
              <template v-for="(title, j) in o['dce:title']" :key="'ctitle' + j">
                <template v-for="(mt, i) in title['bf:mainTitle']" :key="'cmt' + j + '-' + i">
                  <v-col md="3" cols="12" class="pdlabel secondary--text font-weight-bold">
                    ...
                    <v-container>
                      <v-row>
                        <template v-for="(title, j) in series['dce:title']" :key="'stitle' + k + '-' + j">
                          <template v-for="(mt, i) in title['bf:mainTitle']" :key="'smt' + k + '-' + j + '-' + i">
                            <v-col md="3" cols="12" class="pdlabel secondary--text font-weight-bold">{{ $t(title['@type']) }}<template v-if="showLang && mt['@language']"> ({{ mt['@language'] }})</template></v-col>
                      ...
                      <v-row>
                        <template v-for="(volume, i) in series['bibo:volume']" :key="'svol' + k + '-' + i">
                          <v-col md="3" cols="12" class="pdlabel secondary--text font-weight-bold">{{ $t('Volume') }}</v-col>
                          <v-col md="9" cols="12" class="valuefield">{{ volume['@value'] || volume }}</v-col>
                        </template>
                      </v-row>
                      <v-row>
                        <template v-for="(issue, i) in series['bibo:issue']" :key="'sissue' + k + '-' + i">
                          <v-col md="3" cols="12" class="pdlabel secondary--text font-weight-bold">{{ $t('Issue') }}</v-col>
                      ...
                      <v-row>
                        <template v-for="(issued, i) in series['dcterms:issued']" :key="'sissued' + k + '-' + i">
                          <v-col md="3" cols="12" class="pdlabel secondary--text font-weight-bold">{{ $t('Issued') }}</v-col>
                          <v-col md="9" cols="12" class="valuefield">{{ issued }}</v-col>
                        </template>
                      </v-row>
                      <v-row>
                        <template v-for="(issn, i) in series['ids:issn']" :key="'sissn' + k + '-' + i">
                          <v-col md="3" cols="12" class="pdlabel secondary--text font-weight-bold">{{ $t('ISSN') }}</v-col>
