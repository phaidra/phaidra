<template>
  <v-row v-if="o['skos:prefLabel']">
    <v-col class="pdlabel primary--text text-md-right" :md="labelColMd" cols="12">{{ $t(p) }}<template v-if="o['@type'] !== 'schema:Place'"> ({{ $t(o['@type']) }})</template><template v-for="(l) in o['skos:prefLabel']"><template v-if="showLang && l['@language']"> ({{ l['@language'] }})</template></template></v-col>
