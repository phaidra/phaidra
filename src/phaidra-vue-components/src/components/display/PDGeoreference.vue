<template>
  <v-row v-if="o['skos:prefLabel']">
    <v-col class="pdlabel secondary--text font-weight-bold text-md-right" :md="labelColMd" cols="12">{{ $t(p) }}<template v-if="o['@type'] !== 'schema:Place'"> ({{ $t(o['@type']) }})</template><template v-for="(l, li) in o['skos:prefLabel']" :key="'gplang'+li"><template v-if="showLang && l['@language']"> ({{ l['@language'] }})</template></template></v-col>
