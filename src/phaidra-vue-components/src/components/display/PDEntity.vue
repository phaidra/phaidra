  ...
  <v-row v-if="entity">
    ...
    <v-col :md="valueColMd" cols="12">
      <template v-if="entity['@type'] === 'schema:Person'">
        ...
        <template v-else>
          <template v-for="(gn) in entity['schema:givenName']"><span :class="{ 'font-weight-regular': boldLabelFields.includes('role') }">{{ gn['@value'] }}</span></template>
          <template v-for="(fn) in entity['schema:familyName']"><span :class="{ 'font-weight-regular': boldLabelFields.includes('role') }"> {{ fn['@value'] }}</span></template>
          <template v-for="(n) in entity['schema:name']"><span :class="{ 'font-weight-regular': boldLabelFields.includes('role') }">{{ n['@value'] }}</span></template>
        </template>
