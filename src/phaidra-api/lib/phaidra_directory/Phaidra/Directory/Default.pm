...
sub getLDAPEntriesForQuery {
  ...
  my $privateConfig = $confcol->find_one({"config_type" => "private"});
  if ($privateConfig->{ldapextenable}) {
    my $entry_ext = $self->_getLDAPEntriesForQuery($c, $self->get_ldap_ext($c, $privateConfig), $privateConfig->{ldapextusersearchfilter}, $privateConfig->{ldapextusersearchbases}, $query);
