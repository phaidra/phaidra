...
sub post {
  ...
  unless (defined($metadata->{annotations})) {
    $self->render(json => {alerts => [{type => 'error', msg => 'No ANNOTATIONS sent'}]}, status => 400);
    return;
  }

  my $annotations_model = PhaidraAPI::Model::Annotations->new;
  my $res               = $annotations_model->save_to_object($self, $pid, $metadata->{annotations}, $self->stash->{basic_auth_credentials}->{username}, $self->stash->{basic_auth_credentials}->{password}, 0);
