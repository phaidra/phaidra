...
sub record_failed_attempt {
  ...
  eval {$c->mongo->get_collection('rate_limit_attempts')->insert_one($attempt_doc);};
...
sub _cleanup_old_entries {
  ...
  eval {$c->mongo->get_collection('rate_limit_attempts')->delete_many({timestamp => {'$lt' => $cutoff_time}});};
  ...
  eval {$c->mongo->get_collection('rate_limit_blocks')->delete_many({block_until => {'$lt' => $now}});};
...
sub _count_recent_attempts {
  ...
  my $count = 0;
  eval {$count = $c->mongo->get_collection('rate_limit_attempts')->count_documents({identifier => $identifier, timestamp => {'$gt' => $cutoff_time}});};
...
sub _get_block_until {
  ...
  my $block_doc;
  eval {$block_doc = $c->mongo->get_collection('rate_limit_blocks')->find_one({identifier => $identifier});};
...
sub _set_block {
  ...
  eval {$c->mongo->get_collection('rate_limit_blocks')->replace_one({identifier => $identifier}, $block_doc, {upsert => 1});};
...
sub _clear_block {
  ...
  eval {$c->mongo->get_collection('rate_limit_blocks')->delete_one({identifier => $identifier});};
