...
sub list {
  my ($self, $c, $query) = @_;
  $query //= '';
  my $sth  = $self->_dbh($c)->prepare('SELECT username FROM users WHERE username LIKE ? OR email LIKE ? ORDER BY username');
  my $like = '%' . $query . '%';
...
sub get {
  ...
  my $roles = $dbh->prepare('SELECT role FROM user_roles WHERE user_id = ? ORDER BY role');
  ...
  my $affiliations = $dbh->prepare('SELECT affiliation FROM affiliations WHERE user_id = ? ORDER BY affiliation');
...
sub _replace_directory_values {
  ...
  if (ref($data->{affiliation}) eq 'ARRAY') {
    ...
    for my $affiliation (grep {defined && length} @{$data->{affiliation}}) {
      $dbh->do('INSERT INTO affiliations(user_id, affiliation) VALUES(?, ?)', undef, $user_id, $affiliation);
    }
  }
  ...
  if (ref($data->{org_units}) eq 'ARRAY') {
    ...
    for my $org_unit (@{$data->{org_units}}) {
      ...
      $dbh->do('INSERT IGNORE INTO user_org_units(user_id, org_unit_id) VALUES(?, ?)', undef, $user_id, $org_unit);
    }
  }
  ...
  if (ref($data->{roles}) eq 'ARRAY') {
    ...
    for my $role (grep {defined && length} @{$data->{roles}}) {
      $dbh->do('INSERT IGNORE INTO user_roles(user_id, role) VALUES(?, ?)', undef, $user_id, $role);
    }
  }
}
...
sub save {
  ...
  eval {
    ...
    if ($existing) {
      $user_id = $existing->{id};
      my @values       = ($data->{email}, $data->{firstname}, $data->{lastname}, $displayname, $status, $data->{expires_at});
      my $password_sql = '';
      ...
      my $password_hash = defined($data->{password}) && length($data->{password}) ? $self->_hash_password($data->{password}) : undef;
      $dbh->do(
        q{
        INSERT INTO users
...
sub upsert_shib {
  ...
  $self->_dbh($c)->do('UPDATE users SET last_login = CURRENT_TIMESTAMP WHERE username = ?', undef, $data->{username});
...
sub confirm_reset {
  ...
  eval {
    ...
    if ($token) {
      $dbh->do('UPDATE users SET password_hash = ? WHERE id = ?', undef, $self->_hash_password($password), $token->{user_id});
      $dbh->do('UPDATE password_reset_tokens SET used_at = CURRENT_TIMESTAMP WHERE id = ?', undef, $token->{id});
      $success = 1;
    }
    $dbh->commit;
  };
